...
class UNISURFRenderer(L.LightningModule):
    ...
    Args:
        model (nn.Module): model for predicting geometric and semantic features
    ...
    # TODO: was build to render colors (rgb) at specified pixels
    # -> adjust to render semantic features that can be directly compared
    # to the features comming from the foundation model (MaskCLIP)
    ...
    def training_step(self, batch, batch_idx):
        ...
        B, T, C, H, W = image.size()  # TODO: check this

        '''
        # generate rays 
        # -> (not necessary, will be done inside forward())
        focal = (intrinsics[0, 0] + intrinsics[1, 1]) / 2  # TODO
        rays = gen_rays(pose, W, H, focal, z_near=0.1, z_far=8, c=None) # TODO: option to use only a subset (see ray_batch_size in GNFactor)
        '''
        ...
        for image, depth, projection in zip(images, depths, projections):
            ...
            # Sample pixels
            if n_points >= H*W:
                p = arange_pixels((H, W), B)[1].to(device)
                pix = None
            else:
                p, pix = sample_patch_points(B, n_points, patch_size=1., image_resolution=(H, W), continuous=False)
                p = p.to(device) 
                pix = pix.to(device)
            ...
            # TODO: outputs['rgb']: insted of 'rgb' we will render 'features' -> adjust it inside forward()
            outputs['rendered_feat']
            
            # TODO: how to get: output['tsdf'] ? -> for calculating tsdf-loss objective (use the same points that are sampled for the rays?)
            outputs['tsdf']

            targets = {}
            targets['tsdf'] = get_gt_tsdf(batch, p)  # TODO either calculate dynamically (or computed in advance)
                                                     # the tsdf values at the points p from all images of the sequence and
            
            # extract target features from teacher model
            all_features = self.f_teacher(image)
            # use only sampled features to compare with rendered features
            targets['feat'] = get_tensor_values(all_features, pix.clone())
    ...
    def calculate_loss(self, outputs, targets):
        ...
        losses['feat'] = self.loss_feat(outputs, targets)
        
        losses['combined'] = losses['tsdf'] + self.cfg.loss_lambda * losses['feat']
        return losses
