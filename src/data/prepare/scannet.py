...
def prepare_scannet_scene(scene, path, path_meta, verbose=2):
    ...
    data = {'dataset': 'scannet',
            ...
            'scene': scene,
            'file_name_mesh_gt': 
                os.path.join(path_meta, folder, scene,scene+'_vh_clean_2.ply'),
            #'file_name_seg_indices': '',
            #'file_name_seg_groups': '',
            #'instances': None,
            'frames': []
    ...
    for i, frame_id in enumerate(frame_ids):
        ...
        pose = np.loadtxt(os.path.join(path, folder, scene, 'poses', '%d.txt' % frame_id))
...
def prepare_scannet_splits(path, path_meta):
    ...
    In directory path_meta we create a split.txt file for each split containing
