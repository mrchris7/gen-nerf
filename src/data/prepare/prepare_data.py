...
def fuse_scene(path_meta, scene, voxel_size, trunc_ratio=3, max_depth=3,
    ...
    Args:
        ...
        skip_existing: skip creation of individual tsdf and volume files 
...
def clean_info(scene, path_meta):
    # update info json
    info_file = os.path.join(path_meta, scene, 'info.json')
    data = load_info_json(info_file)
...
if __name__ == "__main__":
    ...
    prepare_scannet(
        os.path.expandvars(args.path),
        os.path.expandvars(args.path_meta),
        i,
        n,
        args.test,
        args.max_depth,
