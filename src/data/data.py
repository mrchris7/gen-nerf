...
def map_tsdf(info, data, voxel_types, voxel_sizes):
    ...
    if len(voxel_types)>0:
        for scale in voxel_sizes:
            #print("key:", 'file_name_vol_%02d'%scale)
            #print("load tsdf:", info['file_name_vol_%02d'%scale])
            tsdf = TSDF.load(info['file_name_vol_%02d'%scale], voxel_types)
            data['vol_%02d'%scale] = tsdf
            #data['origin'] = tsdf.origin.squeeze()
    return data
